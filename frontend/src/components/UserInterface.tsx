...
const UserInterface: React.FC<UserInterfaceProps> = ({ backendName }) => {
    ...
    const [updateUser, setUpdateUser] = useState({ id: '', username: '', email: '' });
    ...
    const handleUpdateUser = async (e: React.FormEvent<HTMLFormElement>) => {
      e.preventDefault();
      try {
        await axios.put(`${apiUrl}/users/${updateUser.id}`, { username: updateUser.username, email: updateUser.email });
        setUpdateUser({ id: '', username: '', email: '' });
        setUsers(
          users.map((user) => {
            if (user.id === parseInt(updateUser.id)) {
              return { ...user, username: updateUser.username, email: updateUser.email };
            }
            return user;
          })
        );
